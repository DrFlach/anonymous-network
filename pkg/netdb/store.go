...
func (s *Store) Add(ri *RouterInfo) error {
	...
	expectedHash := sha256.Sum256(ri.SigningPublicKey)
	if ri.RouterHash != expectedHash {
		return fmt.Errorf("invalid RouterInfo: router hash does not match signing key")
	}
